...
fn main() {
    ...
    'driver_loop: while !handle.is_finished() {
        match rx.recv() {
            Ok(Event::LeftPress) => println!("L"),
            Ok(Event::RightPress) => println!("R"),
            Ok(Event::LeftRelease) => println!("l"),
            Ok(Event::RightRelease) => println!("r"),
            Ok(Event::Reset) => println!("====== RESET ======"),
